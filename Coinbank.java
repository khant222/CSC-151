...
public class Coinbank {
	...
	public int remove(int coinType, int requestedCoins) {
		if (isBankable(coinType)){
			...
			int coinsRemoved = maxWithdrawable(coinType, requestedCoins);
			int coinsAfter = coinsBefore - coinsRemoved;
	...
	/**
	 * Private helper function. Given the type and amount of requested coins,
	 * it returns the maximum amount of coins you can withdraw of that type.
	 * @param coinType either 1, 5, 10, or 25 to be valid
	 * @param requestedCoins number of coins to be removed
	 * @return the maximum amount of coins that can be removed
	 */
	private int maxWithdrawable(int coinType, int requestedCoins){
		int howMany = get(coinType);
		if (requestedCoins > howMany){
			return howMany;
		}
		else{
